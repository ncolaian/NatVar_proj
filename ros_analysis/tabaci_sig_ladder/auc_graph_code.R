...
path_ros <- "/Users/nicholascolaianni/Documents/dangl_lab/ros_analysis/tabaci_sig_ladder/"
...
p <- ggplot(csv_list[[1]][csv_list[[1]]$plate == 2,], aes(x=Name, y=AUC))+
...
p <- p + stat_compare_means(method = "t.test", label = "p.signif", ref.group = "100nm", label.y = 7.7)
#p <- p+compare_means(AUC ~ Name, data=csv_list[[1]], method="anova")
p

ggboxplot(csv_list[[1]], x="Name", y="AUC", color = "plate")+stat_compare_means(method="anova")
