...
antag_ros <- "/Users/nicholascolaianni/Documents/dangl_lab/nat_variants_proj/ros_analysis/antag_1nM_10uM_10_18_19/"
...
get_files_from_directories <- function( first_path_ext ) {
  #this portion of the function retrieves the old dataset
  file_list <- c()
  num <- c()
  csv_list <- list()
  file_list <- list.files(path = first_path_ext, pattern = "*auc_data.csv")
  for ( i in 1:length(file_list) ) {
    csv_list[[i]] <- read.csv(paste(first_path_ext, file_list[i], sep = ""), stringsAsFactors = F)
  }
  return(csv_list )
}
...
#ROS
csv_part <- get_files_from_directories(antag_ros)
data_ros <- get_the_combined_dataframe(csv_part, 4)
data_ros <- get_data_frame_rdy_4_plting(data_ros)
data_ros <- na.omit(data_ros)

unique(data_ros$batch)
#batch effect plot
ggplot(data_ros, aes(x=reorder(Name,-AUC, median), y=AUC))+
  geom_boxplot( outlier.shape = NA)+
  ylab("Log10 AUC")+
  xlab("Peptide ID")+
  labs(color="Plate Number")+
  ggtitle("1nM of Pa + 10uM Antagonist")+
  theme(text = element_text(size=14))+
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=.5),
        plot.title = element_text(hjust = 0.5))

ggplot(data_ros[data_ros$Name == "Pa",], aes(x=plate, y=AUC))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter()+
  ylab("Log10 AUC")+
  xlab("ROS Batch")+
...
ggplot(data_ros[data_ros$Name == "Pa",], aes(x=plate, y=AUC, color=plate))+
  geom_boxplot(outlier.shape = NA)+
  ylab("Log10 AUC")+
  xlab("ROS Batch")+
  labs(color="Plate Number")+
...
#I am going to z_score normalize to Pa

pa_means_std <- c() 
for ( i in 1:nlevels(data_ros$plate) ) {
  pa_means_std <- append(pa_means_std, c(i, mean(data_ros$AUC[data_ros$plate == i & data_ros$Name == pos_control]), sd(data_ros$AUC[data_ros$plate == i & data_ros$Name == pos_control])))
}
pa_means_std <- as.data.frame(matrix(pa_means_std, ncol = 3, byrow = T))

z_scores_per_peptide <- c()
for ( i in pa_means_std[,1] ) {
  for ( j in unique(data_ros$Name[data_ros$plate == i]) ) {
    for (f in data_ros$AUC[data_ros$Name == j & data_ros$plate == i ]) {
      z_scores_per_peptide <- append(z_scores_per_peptide, c(j, (f - pa_means_std$V2[i])/pa_means_std$V3[i] ) )
    }
  }
}
...
#MAIN FIGURE
length(unique(z_scores_per_peptide$V1))
ggplot(z_scores_per_peptide, aes(x=reorder(V1,-V2, median), y=V2))+
  geom_boxplot( outlier.shape = NA)+
  ylab("Log10 AUC Pa Z-Score")+
  xlab("Peptide ID")+
  labs(color="Plate Number")+
  ggtitle("ROS Burst 1nM of Pa + 10uM Antagonist")+
  theme(text = element_text(size=14))+
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=.5),
        plot.title = element_text(hjust = 0.5))
...
write.csv(wheel_data,file = "/Users/nicholascolaianni/Documents/dangl_lab/nat_variants_proj/882_data/updated_work_10_23_18/new_clusters/all_nat_variants_ptanorm_wheel_6_30_19.csv" , quote = F, row.names = F)
